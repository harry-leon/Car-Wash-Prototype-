...
const COLOR_IMAGES: Record<string, string> = {
  White: "https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?auto=format&fit=crop&q=80&w=200&h=120",
  Black: "https://images.unsplash.com/photo-1601362840469-51e4d8d58785?auto=format&fit=crop&q=80&w=200&h=120",
    ...
    <div className="px-4 py-8 md:p-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mx-auto w-full max-w-4xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight md:text-4xl text-foreground">My vehicles</h1>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <div className="text-[15px] font-extrabold text-foreground tracking-tight">{v.plate}</div>
                      <div className="mt-0.5 text-[13px] text-muted-foreground font-medium">{v.brandModel}</div>
                      <div className="mt-0.5 text-xs text-muted-foreground/80">{v.type} · {v.color || TYPE_COLORS[v.type]}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 mr-2">
                      <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full hover:bg-primary/10 hover:text-primary" onClick={() => setEditing(v)}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setDeleteId(v.id)}>
      ...
      <DialogContent className="sm:max-w-[500px] rounded-[2rem] border-border/50 bg-card/90 backdrop-blur-2xl shadow-2xl p-0 overflow-hidden">
        <div className="p-8">
          <DialogHeader className="mb-6">
            <DialogTitle className="text-2xl font-bold">{v ? "Edit vehicle" : "Add vehicle"}</DialogTitle>
          ...
          <form onSubmit={submit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="vbrand" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                Brand & model
              </Label>
              <Input
                id="vbrand"
            ...
            <div className="space-y-2">
              <Label htmlFor="vplate" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                License plate
              </Label>
              <Input
                id="vplate"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {VEHICLE_TYPES.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn("rounded-xl p-2 transition-colors", active ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground group-hover:bg-primary/20 group-hover:text-primary")}>
                        <Icon className="h-5 w-5" />
                      </div>
                      <span className={cn("text-xs font-bold", active ? "text-primary" : "text-muted-foreground")}>{t}</span>
                    </button>
                  );
                })}
              </div>
                ...
                {COLORS.map((c) => (
                  <button
                    ...
                    className={cn(
                      ...
                      COLOR_CLASSES[c],
                      color === c ? "ring-2 ring-primary ring-offset-2 scale-110 shadow-md" : "hover:scale-105 opacity-80"
                    )}
                    title={c}
                  />
                ))}
            ...
            <DialogFooter className="mt-8 gap-3 sm:gap-0 pt-4 border-t border-border/50">
              <Button type="button" variant="ghost" onClick={onClose} className="rounded-xl font-semibold hover:bg-accent">
                Cancel
              </Button>
              <Button type="submit" className="rounded-xl font-bold shadow-md shadow-primary/20 hover:-translate-y-0.5 transition-all">
