...
function WaterWaves() {
  return (
    <div className="absolute bottom-0 left-0 right-0 overflow-hidden pointer-events-none h-40">
      <svg viewBox="0 0 1440 120" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" preserveAspectRatio="none">
        <path
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-background via-background to-primary/5 pt-20">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-1/4 -left-1/4 h-[500px] w-[500px] rounded-full bg-blue-500/10 blur-[100px] opacity-50"
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-16 md:px-8 grid gap-12 lg:grid-cols-2 items-center">
        ...
        <div className="space-y-8">
          ...
          >
            ...
            <TrustBadge icon={<Star className="h-4 w-4 fill-primary text-primary" />} label={t("5-Star Lounge", "Phòng chờ 5 sao")} />
          </div>
        </div>
        ...
        >
          <div className="rounded-3xl border border-border/50 bg-card/70 backdrop-blur-xl shadow-2xl p-8 space-y-5">
            ...
            {[
              { emoji: "🚗", title: t("Touchless Wash", "Rửa Không Chạm"), desc: t("German-tech foam protects your paint", "Bọt công nghệ Đức bảo vệ sơn xe") },
              { emoji: "🏆", title: t("5-Star Waiting Lounge", "Phòng Chờ 5 Sao"), desc: t("AC, free drinks & high-speed Wi-Fi", "Máy lạnh, nước miễn phí & Wi-Fi tốc độ cao") },
              { emoji: "⚡", title: t("Real-time Booking", "Đặt Lịch Thời Gian Thực"), desc: t("Guaranteed slot, zero queue", "Không xếp hàng, có chỗ được đảm bảo") },
              { emoji: "💎", title: t("Loyalty Rewards", "Tích Điểm Thưởng"), desc: t("Earn points every wash", "Tích điểm mỗi lần rửa") },
            ].map((item, i) => (
              <div
                key={i}
                className="flex items-start gap-4 p-3 rounded-xl hover:bg-accent/50 transition-colors cursor-default group"
              >
                <div className="text-2xl group-hover:scale-110 transition-transform">{item.emoji}</div>
                <div>
