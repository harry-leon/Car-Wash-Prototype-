      ...
      {menuOpen && (
        <div className="md:hidden border-t border-border/40 bg-background/95 backdrop-blur-xl px-4 py-4 space-y-3">
          {[
            ...
            <a key={item.href} href={item.href} className="block text-sm font-medium py-2 hover:text-primary transition-colors" onClick={() => setMenuOpen(false)}>
              {t(item.en, item.vi)}
            </a>
          ))}
          <div className="flex gap-2 pt-2">
            <Link to="/login" className="flex-1 text-center rounded-full border border-border/60 px-4 py-2 text-sm font-semibold hover:bg-accent transition-colors">{t("Sign In", "Đăng Nhập")}</Link>
            <Link to="/register" className="flex-1 text-center rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors">{t("Register", "Đăng Ký")}</Link>
          </div>
        </div>
      )}
    </header>
